...
def parse() -> Namespace:
    ...
    query_parser = subparsers.add_parser(
        "query", help="Query packages without reading any file"
    )
    query_parser.add_argument("packages", nargs="+", help="Package names to query")
    ...
    file_parser = subparsers.add_parser(
        "file", help="Query packages from selected file and write results into it"
    )
    file_parser.add_argument(
        "-r", "--root", default=".", help="Root of the repository (default '.')"
    )
    file_parser.add_argument(
        "-f",
        "--file",
