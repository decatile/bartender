...
def find_newest_change(repository_root: str, relative_filepath: str) -> datetime:
    L.debug(f"Opening git repository at {os.path.abspath(repository_root)}")
    ...
    for blame_hunk in repo.blame(relative_filepath):
        ...
        if commit is None:
            raise ValueError(
                f"Commit with ID {blame_hunk.orig_commit_id} is referenced by blame hunk, "
    ...
    dt = datetime.fromtimestamp(time_unix, timezone(timedelta(minutes=time_offset)))
